 ...
 *            Initial Coding - Search CIC in ELL38I Search Screen
 **/
package KPC
...
class ELL38C_SEARCH extends GenericScriptPlugin implements GenericScriptExecuteForCollection, GenericScriptExecute {
	String version = "1";
	...
	GenericScriptResults executeForCollection(SecurityToken securityToken, RequestAttributes requestAttributes,
		Integer maxNumberOfObjects, RestartAttributes restartAttributes) throws FatalException{
			log.info("ELL38C_SEARCH executeForCollection : " + version );
			def results = new GenericScriptResults();
			
			RequestAttributes reqAtt = requestAttributes;
			String CNT = reqAtt.getAttributeStringValue("parCntNo");
			String CIC = reqAtt.getAttributeStringValue("parCicNo");
			String WO = reqAtt.getAttributeStringValue("parWo");
			...
			if (CNT.equals(null)) {
				...
				CNT = " and upper(trim(a.CONTRACT_NO)) = upper(trim('" + CNT + "')) ";
			}
			...
			if (CIC.equals(null)) {
				...
				def QRY1;
				QRY1 = sql.firstRow("select * from ACA.KPF38F where DSTRCT_CODE = '"+securityToken.getDistrict()+"' and upper(trim(CIC_NO)) = upper(trim('"+CIC+"')) ");
				log.info ("FIND CIC  : " + QRY1);
				if(QRY1.equals(null)) {
					GenericScriptResult result = new GenericScriptResult()
					StrErr = "INVALID CIC NUMBER"
					SetErrMes();
					com.mincom.ellipse.errors.Error err = new com.mincom.ellipse.errors.Error(CobolMessages.ID_8541)
					err.setFieldId("parCicNo")
					result.addError(err)
					results.add(result)
					RollErrMes();
					return results
				}
				CIC = " and upper(trim(a.CIC_NO)) = upper(trim('" + CIC + "')) ";
			}
			...
			if (WO.equals(null)) {
				...
				def QRY1;
				QRY1 = sql.firstRow("select * from MSF620 where DSTRCT_CODE = '"+securityToken.getDistrict()+"' and upper(trim(WORK_ORDER)) = upper(trim('"+WO+"')) ");
				log.info ("FIND WO  : " + QRY1);
				if(QRY1.equals(null)) {
					GenericScriptResult result = new GenericScriptResult()
					StrErr = "INVALID WO NUMBER"
					SetErrMes();
					com.mincom.ellipse.errors.Error err = new com.mincom.ellipse.errors.Error(CobolMessages.ID_8541)
				...
				WO = " and upper(trim(a.WORK_ORDER)) = upper(trim('" + WO + "')) ";
			}
			...
			if (restartAttributes.equals(null)){
				   StrSQL = "select a.*,case when b.EQUIP_NO is null then ' ' else b.EQUIP_NO end EQP_NO,case when b.DSTRCT_ACCT_CODE is null then ' ' else replace(b.DSTRCT_ACCT_CODE,a.dstrct_code) end ACCT_CODE from ACA.KPF38F a " +
				               "left outer join msf620 b on (a.dstrct_code = b.dstrct_code and a.work_order = b.work_order) " + 
							   "where a.DSTRCT_CODE = '"+securityToken.getDistrict()+"'" + CNT + CIC + WO +
							   "ORDER BY a.CONTRACT_NO,a.CIC_NO DESC,a.WORK_ORDER OFFSET 0 ROWS FETCH NEXT "+maxNumberOfObjects.toString()+" ROWS ONLY" ;
				   log.info ("StrSQL : " + StrSQL);
				   sql.eachRow(StrSQL, {
					   GenericScriptResult result = new GenericScriptResult()
					   result.addAttribute("parGrdCicNo", it.CIC_NO);
					   result.addAttribute("parGrdCicDesc", it.CIC_DESC);
					   if (it.CIC_STATUS.trim().equals("1")) {
						   result.addAttribute("parGrdCicStat", it.CIC_STATUS + " - Estimated");
					   }else if (it.CIC_STATUS.trim().equals("2")) {
						   result.addAttribute("parGrdCicStat", it.CIC_STATUS + " - Accepted");
					   }else if (it.CIC_STATUS.trim().equals("3")) {
						   result.addAttribute("parGrdCicStat", it.CIC_STATUS + " - Canceled");
					   }else if (it.CIC_STATUS.trim().equals("4")) {
						   result.addAttribute("parGrdCicStat", it.CIC_STATUS + " - Invoiced");
					   }else if (it.CIC_STATUS.trim().equals("U")) {
						   result.addAttribute("parGrdCicStat", it.CIC_STATUS + " - Awaiting Approval");
					   }else if (it.CIC_STATUS.trim().equals("R")) {
						   result.addAttribute("parGrdCicStat", it.CIC_STATUS + " - Rejected");
					   }
					   result.addAttribute("parGrdDst", it.DSTRCT_CODE);
					   result.addAttribute("parGrdWoNo", it.WORK_ORDER);
					   result.addAttribute("parGrdCntNo", it.CONTRACT_NO);
					   result.addAttribute("parGrdActDt", it.ACTUAL_DATE);
					   result.addAttribute("parGrdOriginator", it.ORIGINATOR_CIC);
					   result.addAttribute("parGrdCicTyp", it.CIC_TYPE);
					   result.addAttribute("parGrdCumCst", df.format(it.CUM_COST));
					   result.addAttribute("parGrdActCst", df.format(it.ACT_COST));
					   result.addAttribute("parGrdEstCst", df.format(it.EST_COST));
					   result.addAttribute("parGrdCicInv", it.CIC_INVOICE);
					   result.addAttribute("parGrdEstBy", it.EST_BY);
					   result.addAttribute("parGrdEstDate", it.EST_DATE);
					   result.addAttribute("parGrdActBy", it.ACT_BY);
					   result.addAttribute("parGrdActDate", it.ACTUAL_DATE);
					   result.addAttribute("parGrdAppBy", it.COMPL_BY);
					   result.addAttribute("parGrdAppDt", it.COMPL_DATE);
					   result.addAttribute("parGrdEqpNo", it.EQP_NO);
					   result.addAttribute("parGrdAcctCode", it.ACCT_CODE);
				   ...
				   StrSQL = "select a.*,case when b.EQUIP_NO is null then ' ' else b.EQUIP_NO end EQP_NO,case when b.DSTRCT_ACCT_CODE is null then ' ' else replace(b.DSTRCT_ACCT_CODE,a.dstrct_code) end ACCT_CODE from ACA.KPF38F a " +
				               "left outer join msf620 b on (a.dstrct_code = b.dstrct_code and a.work_order = b.work_order) " + 
							   "where a.DSTRCT_CODE = '"+securityToken.getDistrict()+"'" + CNT + CIC + WO +
							   "ORDER BY a.CONTRACT_NO,a.CIC_NO DESC,a.WORK_ORDER OFFSET "+MaxInst.toString()+" ROWS FETCH NEXT "+maxNumberOfObjects.toString()+" ROWS ONLY" ;
				   log.info ("StrSQL : " + StrSQL);
	...
	@Override
	GenericScriptResults execute(SecurityToken securityToken, List<RequestAttributes> requestAttributes, Boolean aBoolean) throws FatalException {
		...
		String reportID = reqAtt.getAttributeStringValue("reportId") ? reqAtt.getAttributeStringValue("reportId").trim() : null
		...
		if(!QRY0.equals(null)) {
			...
			if(QRY0.SEC_389.trim().equals("FALSE")) {
				StrErr = "YOU DO NOT HAVE ACCESS TO KPJ389"
				SetErrMes();
				com.mincom.ellipse.errors.Error err = new com.mincom.ellipse.errors.Error(CobolMessages.ID_8541)
				//err.setFieldId("CRE_CNT_NO")
				result.addError(err)
...
//	region PRT_RPT
	private String invoke_Report(){
		...
		if (screen.mapname.trim() == new String("MSM080A")) {
