...
* @EMS Mar 2019
*
* 20190320 - a9ra5213 - Ricky Afriano - KPC UPGRADE
*            Initial Coding - Display supplier data into dropdown component in ELL38I Screen
**/
...
public class SUPP_LIST extends GenericScriptPlugin implements GenericScriptExecuteForCollection{
   String version = "1";
   ...
   public GenericScriptResults executeForCollection(SecurityToken securityToken, RequestAttributes requestAttributes,
	   ...
	   log.info("Exec for coll SUPP_LIST : " + version )
	   ...
	   if(strSearch.equals(null)){
		   ...
		   if (restartAttributes.equals(null)){
			   StrSQL = "select row_number () over(order by SUPPLIER_NO) NO,trim(SUPPLIER_NO) SUPPLIER_NO,trim(SUPPLIER_NAME) SUPPLIER_NAME from msf200 " +
						   "ORDER BY SUPPLIER_NO OFFSET 0 ROWS FETCH NEXT "+maxNumberOfObjects.toString()+" ROWS ONLY" ;
			   log.info ("StrSQL : " + StrSQL);
			   sql.eachRow(StrSQL, {
				   GenericScriptResult result = new GenericScriptResult();
				   result.addAttribute("resCode", it.SUPPLIER_NO);
				   result.addAttribute("resDesc", it.SUPPLIER_NAME);
			   ...
			   StrSQL = "select row_number () over(order by SUPPLIER_NO) NO,trim(SUPPLIER_NO) SUPPLIER_NO,trim(SUPPLIER_NAME) SUPPLIER_NAME from msf200 " +
						   "ORDER BY SUPPLIER_NO OFFSET "+MaxInst.toString()+" ROWS FETCH NEXT "+maxNumberOfObjects.toString()+" ROWS ONLY" ;
			   log.info ("StrSQL : " + StrSQL);
			   sql.eachRow(StrSQL, {
				   GenericScriptResult result = new GenericScriptResult();
				   MaxInst = it.NO
		   ...
		   if (restartAttributes.equals(null)){
			   StrSQL = "select row_number () over(order by SUPPLIER_NO) NO,trim(SUPPLIER_NO) SUPPLIER_NO,trim(SUPPLIER_NAME) SUPPLIER_NAME from msf200 " +
						   "where (upper(SUPPLIER_NO) like '%'||upper('"+strSearch+"')||'%') or (upper(SUPPLIER_NAME) like '%'||upper('"+strSearch+"')||'%') " +
						   "ORDER BY SUPPLIER_NO OFFSET 0 ROWS FETCH NEXT "+maxNumberOfObjects.toString()+" ROWS ONLY" ;
			   log.info ("StrSQL : " + StrSQL);
			   sql.eachRow(StrSQL, {
				   GenericScriptResult result = new GenericScriptResult();
