...
class ApprovalHistoryService_search extends ServiceHook{
	...
	Object onPreExecute(Object input) {
		...
		if (c.getTran877Type().getValue() == "VA"){
			...
			custAttribs.each{Attribute customAttribute ->
				...
				if (customAttribute.getName() == new String("contractNo")){
					CNT_NO = customAttribute.getValue()
				}
				if (customAttribute.getName() == new String("cicNo")){
	...
	Object onPostExecute(Object input, Object result) {
		...
		if (c.getTran877Type().getValue() == "VA"){
			...
			for (i = 0 ; i < d.length ; i++) {
				...
				if(TransKey.trim() == TRANS_DTO.getTransactionKey().value.trim()) {
					CTR = CTR + 1
				}
			}
			//copy array
